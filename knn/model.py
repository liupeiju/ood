...
class ContrastiveMoCoKnnBert(nn.Module):
    ...
    def forward_by_multi_v2(self,
        ...
        q = bert_output_q[1]
        liner_q = self.contrastive_liner_q(q)
        logits_cls = self.classifier_liner(line_q)
    ...
    def forward(self,
        ...
        if mode == 'train':
            ...
            return torch.argmax(logits_cls, dim=1).tolist(), labels.cpu().numpy().tolist()
...
class ContrastiveMoCoKnnInitByBert(nn.Module):
    ...
    def forward_no_multi_head(self,
        ...
        """
        update_num = 16 K = 32000
        positive sample 16(用于更新queue, top_k 取100)
        前200和倒1，真实neg 3090-0  70.397
        前200和倒200，真实neg 3090—0 71.48
        前100和倒1， 真实neg 3090-1 knn 71.58 cls 71.48 72.20 第10个epoch 2张3090
        前100和倒1， 真实neg 3090-0 knn 71.11 cls 69,67 71.48 第11个epoch 4张3090 
        前100和倒1， 全部neg 3090-1 knn 47 cls 70.03
        前100和倒1， 全部neg 3090-0 knn 47 cls 68.5
        update_num = 1 K 2000
        前100和倒1， 真实neg 3090-1 knn 70.39 cls 71.11 70.75 第10个epoch 2张3090
        """
