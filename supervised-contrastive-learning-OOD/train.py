...
def parse_args():
    ...
    parser.add_argument("--ind_pre_epoches", type=int, default=2,
                        help="Max epoches when in-domain pre-training.")
    parser.add_argument("--supcont_pre_epoches", type=int, default=2,
                        help="Max epoches when in-domain supervised contrastive pre-training.")
    parser.add_argument("--aug_pre_epoches", type=int, default=2,
...
df, partition_to_n_row = load_data("CLINC")
...
#if 'CLINC' in args.dataset and not args.n_plus_1:
    n_class -= 1
...
train_save = list(train_text)
valid_save = list(valid_text)
test_save = list(test_text)
...
if args.mode in ["train", "both"]:
    ...
    if args.sup_cont:
        for epoch in range(1,args.supcont_pre_epoches+1):
            ...
            for j, (seq, label) in enumerate(train_iterator):
                ...
                train_iterator.set_description('sup contrastive Iter (sup_cont_loss=%5.3f)' % (loss.item()))
                losses.append(loss)
                optimizer.zero_grad()
                loss.backward()
    ...
    for epoch in range(1,args.ind_pre_epoches+1):
        ...
        for j, (seq, label) in enumerate(train_iterator):
            ...
            train_iterator.set_description('ind_pre Iter (ce_loss=%5.3f)' % (loss.item()))
            losses.append(loss)
            optimizer.zero_grad()
            loss.backward()
            torch.nn.utils.clip_grad_norm_(model.parameters(), args.clip)
    ...
    if args.augment:
        for epoch in range(1, args.aug_pre_epoches + 1):
            global_step1 = 0
            losses = []
            augment_loader = DataLoader(train_augment, BATCH_SIZE, mode='augment', use_bert=USE_BERT)
            augment_iterator = tqdm(
                augment_loader, initial=global_step1,
                desc="Iter (loss=X.XXX)")
            model.train()
            for j, (seq, aug_seq) in enumerate(augment_iterator):
                inputs = torch.zeros((2*seq.size(0),seq.size(1)))
                for e in range(seq.size(0)):
                    inputs[2*e] = seq[e]
                    inputs[2*e+1] = aug_seq[e]
                if args.cuda:
                    inputs = inputs.cuda()
                inputs = inputs.long()
                adv_features = PGD_contrastive(model, inputs, alpha=args.norm_coef/255.)
                loss = model(inputs, adv_features, None, 'augment')
                ...
                optimizer.step()
                global_step1 += 1
            print('Epoch: [{0}] :  Loss {loss:.4f}'.format(epoch, loss=sum(losses) / global_step1))

            global_step2 = 0
            losses = []
            train_loader = DataLoader(train_data, BATCH_SIZE, use_bert=USE_BERT, raw_text=train_text)
            train_iterator = tqdm(
                train_loader, initial=global_step2,
                desc="Iter (loss=X.XXX)")
            valid_loader = DataLoader(valid_data, BATCH_SIZE, use_bert=USE_BERT, raw_text=valid_text)
            model.train()
            for j, (seq, label) in enumerate(train_iterator):
                if args.cuda:
                    if not USE_BERT:
                        seq = seq.cuda()
                    label = label.cuda()
                loss = model(seq, None, label, 'ind_pre')
                train_iterator.set_description('Iter (loss=%5.3f)' % (loss.item()))
                losses.append(loss)
                optimizer.zero_grad()
                loss.backward()
                ...
                global_step2 += 1
            print('Epoch: [{0}] :  Loss {loss:.4f}'.format(
                epoch, loss=sum(losses)/global_step2))
            ...
            for j, (seq, label) in enumerate(valid_loader):
                if args.cuda:
                    if not USE_BERT:
                        seq = seq.cuda()
                    label = label.cuda()
                output = model(seq, None, label, 'validation')
                predict += output[0]
                target += output[1]
            f1 = metrics.f1_score(target, predict, average='macro')
            if f1 > best_f1:
...
if args.save_embed:
    ...
    def save_embed(text, filename):
        start = 0
        hts = []
        while True:
            end = min(len(text), start+BATCH)
            chunk = text[start:end]
            ht = model.save_embed(tuple(chunk))
            hts.append(ht)
            start = end
            if start == len(text):
                break
        torch.save(torch.cat(hts, dim=0), './pts/' + filename + '_naive.pt')
        print('%s embedding is saved' % filename)

    save_embed(train_save, 'train')
    save_embed(valid_save, 'valid')
    save_embed(test_save, 'test')